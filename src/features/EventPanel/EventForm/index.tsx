...
const EventForm = ({ context, onClose, editing, onFinish }: IProps) => {
  ...
  const pageListOptions = context.pageList.map((cur: IPageConfig) => ({
    label: cur.title,
    value: cur.id,
  }));
    ...
    >
      ...
      <EventFormItem label="动作类型" value="type">
        <SchemaSelect
          ...
          options={EVENT_TYPE}
          value={type}
          onChange={(value: string) => form.setFieldValue("type", value)}
        >
          {EVENT_TYPE.map((cur: any) => (
      ...
      {(type === "open-modal" || type === "close-modal") && (
        <EventFormItem label="弹窗" value="modalId">
          <SchemaSelect
            ...
            onChange={(value: string) => form.setFieldValue("modalId", value)}
          >
            {modalOptions.map((cur: any) => (
              <option value={cur.value}>{cur.label}</option>
            ))}
          </SchemaSelect>
        </EventFormItem>

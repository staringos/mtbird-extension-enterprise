...
import {IComponentInstance, IComponentLibs, IExtensionFeatureProps} from '@mtbird/shared';
...
const ComponentLibsPanel = ({context}: IExtensionFeatureProps) => {
  ...
  const list = values(registeredComponents).filter((cur: IComponentInstance) => cur.componentLib && cur.componentLib === currentLib)

  console.log("rrrr registeredComponents:", registeredComponents)
  console.log("rrrr cccc currentLib:", currentLib)

  const options = componentLibs ? componentLibs.map((cur: any) => ({label: cur.title, value: cur.key})) : []

  return (
    <div className={styles.componentLibsWrapper}>
      <div>
        <SchemaSelect value={currentLib} options={options} onChange={handleChange} />
      </div>
      <div className={styles.componentList}>
        {registeredComponents && <ToolBoxList list={list} onItemClick={handleAddComponent} />}
      </div>
    </div>
  );
};
